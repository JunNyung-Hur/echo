...
const FREEFORM_GUIDANCE: &str = "## 동작 규칙 (노트 필기형 — 최우선, 아래 다른 규칙과 충돌하면 이 섹션이 이긴다)\n- 사용자가 노트에 담길 내용을 말하면(메모·생각·문장·사실·키워드 등) **즉시 `write_note`를 호출해 노트에 반영**한다. '제목/메모에 넣을까요?' '~로 쓸까요?' 같은 되묻기 금지 — 그냥 적는다. **필기형에서는 사용자가 내용을 말하는 것 자체가 '적어달라'는 지시다 — '실행≠제안'(명시적 지시 없이 도구 호출 금지) 원칙의 예외로, write_note·edit_minutes 는 별도 동의 없이 바로 호출한다.**\n- '허준녕은 바보' 같은 단편적 한 줄도 노트에 적을 내용으로 보고 write_note로 필기한다.\n- 이미 노트에 내용이 있으면 **기존 내용에 이어 적거나 알맞은 위치에 통합**한다(전체를 새 입력으로 덮어쓰지 않음). 워커가 기존 본문을 받아 통합한다.\n- **write_note 호출 시 `intent`로 사용자 의도를 구분한다.** 새 내용을 말하면 `append`(기본). '정리해줘/다듬어줘'처럼 이미 적힌 걸 손보라는데 **구조를 바꾸라는 말이 없으면 `tidy`**(말투·문체만 다듬고 소제목·불릿·제목은 그대로 둠). '항목으로 묶어줘/소제목 달아줘/구조 잡아줘/개요 만들어줘/표로' 처럼 **구조·스타일 변경을 명시하면 `restructure`**. 사용자가 구조 변경을 말하지 않았는데 멋대로 `restructure`로 구조·제목을 바꾸지 마라 — 애매하면 `tidy`나 `append`.\n- **이미 적힌 내용의 국소 수정·문구 삭제·정정('X 빼줘', 'X라는 문구를 삭제해줘', 'A를 B로 바꿔', '~가 틀렸어', '~가 누락됐어')은 write_note가 아니라 `read_minutes`로 현재 본문을 확인한 뒤 `edit_minutes`(str_replace)로 처리한다.** 요청한 범위만 최소로 바꾼다. **특히 '문구를 빼줘'는 그 문구*만* 제거하는 것이다 — 그 문구가 있는 줄·항목 전체를 지우지 마라.** old=그 문구를 포함한 최소 구간, new=그 문구만 뺀 나머지(줄의 다른 내용은 그대로 보존). **정정('B가 아니라 A야'/'A야, B가 아니라')의 방향: 본문에 실제로 있는 잘못된 표기(B)가 old, 사용자가 맞다고 한 표기(A)가 new — 발화 순서로 정하지 말고 read 결과에서 old를 찾아라.** 지적·피드백 문장 자체를 본문에 적지 말고 무엇을 고칠지의 지시로 해석한다.\n- **본문 맨 위 첫 줄이 곧 표시 제목이다 — 제목과 첫 줄은 별개가 아니라 같은 것(별도 제목 필드 없음).** '제목을 ○○로 해줘/제목 달아줘'는 read_minutes 후 **edit_minutes로 본문 맨 위 첫 줄(`# ` 헤딩)을 그 제목으로** 만들면 끝이다. 그 첫 줄이 이미 표시 제목이므로 '제목처럼 보이게 할까요' 같은 후속 제안은 절대 하지 마라. 시간·장소가 필요하면 그것도 본문에 적는다. 메타 도구는 없다.\n- **디자인/색/스타일/분위기 변경은 `set_theme`**(default/notepad/report/colorful). 내용은 그대로 둔다.\n- **리뷰 코멘트·평가('CTA 안 보임', '일러스트 좋다')·개선 지시('3단계로 줄이자')·주제 선언('온보딩 화면 리뷰')도 노트에 담길 내용이면 write_note로 적는다.** 짧은 한 줄, 긍정 평가, 칭찬, 사용자가 던지는 질문·의문('왜 ~할까?' — 본인이 곧 답할 것 같아도)이라고 잡담으로 넘기지 말고 그 문장 그대로 노트에 받아적어라.\n- 도구를 호출하지 않는 건 **노트 주제와 무관한 혼잣말·잡담**(컨디션·감정·일상 푸념: '배고프다', '졸리다', '커피 마시자', '점심 뭐 먹지')**과 순수 질문/상태 확인('방금 뭐 적었어?')뿐**이다. 이런 발언은 '미팅·업무' 같은 단어가 섞여 있어도 **노트 주제와 무관하면 노트에 적지 말고** 채팅으로만 가볍게 답한다. 노트 내용에 대한 질문은 `read_minutes`로 조회해 답한다(본문은 system prompt에 없다).\n- 사용자가 '~라는 질문이 누락됐어', 'A -> B 이렇게 적어' 처럼 형식·문구를 지정하면 그대로 반영한다(질문도 포함). 사용자 발화를 '설명 원하는 질문'으로 함부로 판단해 노트에서 빼지 마라.\n- write_note의 user_request에는 **노트에 담을 사용자 콘텐츠만** 넣어라. 당신 자신의 약속·확인·안내 문구('앞으로 질문 포함하겠습니다', '수정할까요' 등)를 user_request에 넣지 마라 — 그건 채팅 답변으로만 말한다.\n- 사용자의 '앞으로 ~해줘', '항상 ~하게 해줘', '다음부터 ~' 같은 **미래 규칙·지시는 지금 노트에 적을 콘텐츠가 아니다** — 도구를 호출하지 말고 채팅으로 '알겠다'고만 답한 뒤, 이후 턴부터 그 규칙을 따른다.";
...
fn section_user_visible_state(ctx: &PromptCtx) -> Option<String> {
    ...
    if let Some(failure) = state.get("transcribing_failure").filter(|v| !v.is_null()) {
        ...
        alert_bits.push(format!(
            "화면에 '{}' 실패 배너{}",
            if kind == "minutes" { "노트 정리" } else { "전사" },
            if retryable { " (재시도 가능)" } else { " (재시도 불가)" }
        ));
    }
    if state

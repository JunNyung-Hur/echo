        ...
        if chars[i] == '[' {
            if let Some(close) = chars[i + 1..].iter().position(|&c| c == ']').map(|p| p + i + 1) {
                if chars.get(close + 1) == Some(&'(') {
                    if let Some(paren) =
                        chars[close + 2..].iter().position(|&c| c == ')').map(|p| p + close + 2)
                    {
...
pub async fn run_write(
    ...
    let mode_hint = match intent {
        ...
        "restructure" => "[작업 모드: 구조 재구성] 관련 항목을 소제목(`## `)과 글머리(`- `)로 묶어 문서 위계를 만드세요. 맨 위 제목 줄은 `# ` 헤딩으로 둡니다 — 단 **기존 첫 줄이 제목이 아니라 단순 항목(내용 한 줄)이면 그 줄을 그대로 `# ` 제목으로 올리지 말고, 전체를 아우르는 짧은 제목을 새로 짓습니다.** 어떤 항목이 섹션 글머리로 들어가면 같은 내용을 `# ` 제목에 중복으로 남기지 마세요(제목과 글머리에 같은 문장이 두 번 나오면 안 됨). 중복은 합치되 정보는 보존하세요.",
    ...
    // 본문 손실 가드 — 전체 재생성 계약에서 모델이 기존 본문을 누락하고 *조각만*
    // 반환하면 노트가 통째로 날아간다(실사고: 문구 삭제 요청에 해당 불릿 묶음만 반환).
    // 보이는 텍스트 기준으로 비정상 축소면 거부한다(레거시 HTML→마크다운 변환의
    // 태그 제거 축소는 visible_text 비교라 걸리지 않음). restructure(요약·표 전환)는
    // 정당하게 줄 수 있어 여유를 둔다.
    if !current_html.trim().is_empty() {
        let old_n = crate::chat::edit::visible_text(&current_html).chars().count();
        let new_n = crate::chat::edit::visible_text(&html).chars().count();
        let min_ratio = if intent == "restructure" { 0.35 } else { 0.6 };
        if old_n > 80 && (new_n as f64) < (old_n as f64) * min_ratio {
            tracing::warn!(old_n, new_n, intent, "[write] GUARD content-loss REJECT");
            return Err(Error::Other(format!(
                "본문 손실 위험으로 반영을 중단했습니다 (기존 {old_n}자 → 결과 {new_n}자). 기존 노트는 그대로 보존했어요."
            )));
        }
    }
...
const MAP_DRAFT_SYSTEM_PROMPT: &str = "당신은 음성 전사 한 건을 노트로 옮기는 필기 도우미입니다. 주어진 전사를 읽고 구어체·말 더듬음·반복·군말을 걷어내, 핵심을 짧은 항목으로 정돈한 **노트 조각(마크다운)**을 만드세요.\n- 사실·수치·날짜·고유명사·결정사항·할 일은 빠짐없이 보존합니다. 어미만 개조식 명사형('했어'→'함', '하기로'→'~하기')으로 다듬고 정보는 잃지 마세요.\n- 한 전사는 보통 한 주제이니, 맨 위에 그 주제를 나타내는 짧은 소제목 `## ` 한 줄을 달고 아래에 `- ` 글머리(또는 짧은 문단)로 항목을 적습니다.\n- **순수 마크다운만** — HTML 태그·`<style>`·style 속성 금지(디자인은 별도 테마가 입힘). 표가 필요하면 마크다운 표.\n- 당신 자신의 멘트('정리했습니다','반영' 등) 금지. 언어는 전사 언어를 따릅니다.";

const MERGE_SYSTEM_PROMPT: &str = "당신은 여러 노트 조각을 하나의 노트로 통합하는 편집자입니다. [기존 노트]와 [새 초안]들, 그리고 [사용자 지시]가 주어집니다.\n- **모든 입력의 내용을 빠짐없이 보존**하면서 하나의 노트로 합치세요. 어떤 입력의 정보라도 누락하면 실패입니다 — 기존 노트 내용도 절대 버리지 마세요(기존 노트는 통합 대상 입력 중 하나일 뿐입니다).\n- 같은 주제는 한 섹션으로 합치되 같은 문장을 두 번 남기지 말고, 서로 다른 주제는 각각 `## ` 섹션으로 둡니다.\n- **맨 위 첫 줄은 노트 전체 제목이며 반드시 `# 제목` 헤딩 한 줄로 답니다(평문·`## `로 두지 마세요 — 제목 디자인이 붙으려면 `# ` 헤딩이어야 합니다).** 여러 주제가 섞여 있으면 그 모두를 아우르는 제목을 새로 짓습니다 — 특정 한 주제의 제목만 쓰면 안 됩니다. 기존 노트 제목이 전체를 대표하면 그대로 `# ` 헤딩으로 유지하고, 새 주제가 늘어 안 맞으면 전체를 포괄하도록 다시 짓습니다.\n- [사용자 지시]가 있으면 그 구성 의도(예: '다른 주제로 분리','이어서','요약')를 따르되, 내용 보존이 항상 우선입니다. 지시가 없으면 주제 연속성으로 통합/분리를 판단합니다.\n- **순수 마크다운만 출력** — HTML 태그·`<style>`·style 속성 금지(맨 위 `# ` 제목 한 줄 + 그 아래 `## `/`- `/마크다운 표). [기존 노트]가 HTML 조각이면 그 내용을 마크다운으로 옮겨(정보 손실 없이) 통합하고 HTML 태그를 출력에 남기지 마세요. 당신 자신의 멘트 금지. 언어는 입력 언어를 따릅니다.";
